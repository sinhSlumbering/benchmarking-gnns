...
for nodes in "${!node_samples[@]}"; do
    ...
    python data/TSP/generate_TSP.py \
        ...
        --filename "${base_filename}${nodes}-${nodes}_train/tsp${nodes}-${nodes}_train.txt" \
        --node_dim 2 \
        --seed "$seed"
    ...
    python data/TSP/generate_TSP.py \
        ...
        --filename "${base_filename}${nodes}-${nodes}_val/tsp${nodes}-${nodes}_val.txt" \
        --node_dim 2 \
        --seed "$val_seed"
    ...
    python data/TSP/generate_TSP.py \
        ...
        --max_nodes "$nodes" \
        --num_samples "$val_test_samples" \
        --filename "${base_filename}${nodes}-${nodes}_test/tsp${nodes}-${nodes}_test.txt" \
        --node_dim 2 \
        --seed "$test_seed"
